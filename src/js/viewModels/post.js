define([
  ...
  function postModel() {
    ...
    self.createPost = () => {
      const category_id = self.category_id();
      const post_title = self.post_title();
      const post_body = self.editor().getData();
      $.ajax({
        url: `${RESTurl}`,
        headers: {
          Authorization: "Bearer " + userToken,
    ...
    self.updatePost = () => {
      const category_id = self.category_id();
      const post_id = self.post().id;
      const post_title = self.post().post_title;
      const post_body = self.edit().getData();
      $.ajax({
        url: `${RESTurl}/${post_id}`,
        headers: {
          Authorization: "Bearer " + userToken,
