  ...
  function CategoryViewModel () {
    ...
    self.selectedCategoryChanged = function (event) {
      ...
      if (event.detail.value.length != 0) {
        ...
        const { data } = self.firstSelectedCategory()
        if (data == null) {
    ...
    self.selectedPostChanged = function (event) {
      ...
      if (event.detail.value.length != 0) {
        // If selection, populate and display Category details
        // Populate items list observable using firstSelectedXxx API
        const { data } = self.firstSelectedPost()
        self.postData(data)
        self.viewPostModal()
      }
    }
    ...
    self.createCategory = function (event, data) {
      const title = self.newCategory.category_name
      const description = self.newCategory.description
      $.ajax({
        url: `${RESTurl}`,
        headers: {
    ...
    self.fetchCategories = function () {
      ...
      const numberOfPosts = category_id => {
        $.ajax({
    ...
    self.updateCategorySubmit = function (event) {
      ...
      const title = self.categoryData().category_name
      const description = self.categoryData().dsecription
    ...
    self.deleteCategory = function (event, data) {
      ...
      const title = self.firstSelectedCategory().data.category_name
      $.ajax({
