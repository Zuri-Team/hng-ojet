  ...
  function TeamViewModel () {
    ...
    self.selectedTeamChanged = function (event) {
      ...
      if (event.detail.value.length != 0) {
        ...
        const { data } = self.firstSelectedTeam()
        if (data == null) {
    ...
    self.selectedTeamMemberChanged = function (event) {
      ...
      if (event.detail.value.length != 0) {
        // If selection, populate and display Category details
        // Populate items list observable using firstSelectedXxx API
        const { data } = self.firstSelectedTeamMember()
    ...
    self.createTeam = function (event, data) {
      const team_name = self.newTeam.team_name
      const max_team_mates = self.newTeam.max_team_mates
      const team_description = self.newTeam.team_description
      $.ajax({
        url: `${RESTurl}`,
        headers: {
    ...
    self.updateTeamSubmit = function (event) {
      ...
      const team_name = self.firstSelectedTeam().data.team_name
      const max_team_mates = self.firstSelectedTeam().data.max_team_mates
      const team_description = self.firstSelectedTeam().data.team_description
      $.ajax({
        url: `${RESTurl}/${teamId}`,
        headers: {
          Authorization: 'Bearer ' + userToken
